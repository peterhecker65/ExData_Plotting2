## Plotting Assignment 2 for Exploratory Data Analysis / Plot 5
##
...
baltimore_on_road_NEI <- NEI[NEI$fips == '24510' & NEI$type == 'ON-ROAD', ]
baltimore_on_road_NEI_sum_emissions_by_year <-
    aggregate(Emissions ~ year, baltimore_on_road_NEI, sum)

g <-
    ggplot(baltimore_on_road_NEI_sum_emissions_by_year,
           aes(year, Emissions))
g <- g + geom_line() + geom_point()
g <- g + xlab('The year of emissions recorded')
g <- g + ylab('Amount of PM2.5 emitted in tons')
g <-
    g + ggtitle('Emissions from Motor Vehicle Sources in Baltimore City from 1999 to 2008')
